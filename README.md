...
**@After :** @After는 모든 테스트가 종료되면 호출된다. 메모리에서 Resource를 Release 할 수 있다. 

**@Test :** @Before가 완료되면 실제 코드 테스트를 진행하게 된다. 
...
**assertEquals() :** 두 기본형/객체가 동일한지 확인한다.

**assertTrue(조건) :** 조건이 참인지 확인한다.

**assertFalse(조건) :** 조건이 거짓인지 확인한다.

**assertNotNull(object) :** 객체가 Null이 아닌지 확인한다.

**assertNull(object) :** 객체가 Null인지 확인한다.

**assertSame(expected, actual) :** 두 객체의 참조값이 같은지 확인한다.

**assertNotSame(unexpected, actual) :** 두 객체의 참조값이 같지 않은지 확인한다.

**assertArrayEquals(expectedArray, actualArray) :** 두 배열이 동일한지 아닌지를 확인한다.
