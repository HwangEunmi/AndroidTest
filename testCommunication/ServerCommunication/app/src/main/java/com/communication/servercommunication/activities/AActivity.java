                            ...
                            if(DBManager.getInstance(AActivity.this).getSOSData().getCount()  < 0) {
                                ...
                                try {
                                    for (int i = 0; i < request.mData.size(); i++) {
                                        ...
                                        tempCursorValue = DBManager.getInstance(AActivity.this).getKeywordSOSData(request.mData.get(i).mAnsim_info_seq);
                                        ...
                                        if (ss != tempInt[i]) {
                                            /*DB에 저장*/
                                            mDBManager.insertSOSData(request.mData.get(i));
            ...
            public BaseData onFail(BaseData data) {
                if (data != null) {
                    ...
                    if ("false".equals(request.mIsSuccess)) { // 실패한 경우
                        if (request.mData != null) {
                            Toast.makeText(AActivity.this, String.valueOf(request.mMessage), Toast.LENGTH_SHORT).show();
