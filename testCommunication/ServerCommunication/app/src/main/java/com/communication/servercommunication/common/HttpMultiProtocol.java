...
public class HttpMultiProtocol extends AsyncTask<BaseData, Void, BaseData> {
    ...
    private final String mLINE_FEED = "\r\n";

    private final String mCHARSET = "UTF-8";

    private final String mTWO_HYPHENS = "--";

    private final String mBOUNDARY = mTWO_HYPHENS + System.currentTimeMillis() + mTWO_HYPHENS;

    /*onPostExecute()메소드에 사용 될 인터페이스
    * (onPostExecute()는 뷰 변경 못하니까)*/
    private Utils.NetworkCheckCallback mCallback;
        ...
        try {
            ...
            if (protocol.getmServiceUrl().equals(Constant.GET_BOARD_LIST_URL)) {
                ...
                * opstream.close();
                ...
                /*서버통신 메소드 호출*/
                setURLsetting(path, protocol);
                mUrlConnection.setRequestProperty("Content-Type", "multipart/form-mData; boundary=" + mBOUNDARY);

                OutputStream opstream = mUrlConnection.getOutputStream();
                mWriter = new PrintWriter(new OutputStreamWriter(opstream, mCHARSET), true);
    ...
    protected void onPostExecute(BaseData protocol) {
        ...
        if (mCallback != null) {
            mCallback.onSuccess(protocol);
            mCallback.onFail(protocol);
        }
    ...
    private void addFormField(PrintWriter writer, String name, String value) {
        writer.append(mLINE_FEED);
        writer.append("--" + mBOUNDARY).append(mLINE_FEED);
        writer.append("Content-Disposition: form-mData; name=\"" + name + "\"").append(mLINE_FEED);
        writer.append("Content-Type: text/plain; charset=" + mCHARSET).append(mLINE_FEED);
        writer.append(mLINE_FEED);
        writer.append(value);
        writer.flush();
    }
    ...
    private void addBoundary(PrintWriter writer) {
        writer.append(mLINE_FEED).flush();
        writer.append("--" + mBOUNDARY + "--").append(mLINE_FEED);
        writer.close();
    }
}
